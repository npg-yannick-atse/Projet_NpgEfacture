...
exports.saveDownloadedInvoice = async (req, res) => {
  try {
    ...
    // Cas limite (supprimée puis re-téléchargée) : si ce numéro a déjà un envoi
    // réussi, on initialise is_sent=1 pour qu'elle n'apparaisse pas en "non envoyées".
    const dejaEnvoyee = await LogsAction.findOne({
      where: { numero_facture: numero, SendBy: { [Op.ne]: null }, erreur: false },
      attributes: ['id'],
    });
...
// Téléchargement EN MASSE : enregistre tout un lot de factures en 1 seul appel
// (bulkCreate + logs groupés en une transaction) au lieu de 2 requêtes par facture.
// Cible : factures SAP normales. Les templates (id TMP_) et avoirs gardent le flux
// unitaire (POST /downloaded-invoices) qui gère leurs cas particuliers.
exports.bulkSaveDownloadedInvoices = async (req, res) => {
  try {
    const { invoices, username } = req.body;
    if (!Array.isArray(invoices) || invoices.length === 0) {
      return res.status(400).json({ error: 'Liste de factures requise (invoices)' });
    }

    const numeros = [...new Set(invoices.map(i => i && i.numero).filter(Boolean))];

    // Déjà téléchargées (par numéro) — à ignorer
    const existingRows = await DownloadedInvoice.findAll({
      where: { numero: { [Op.in]: numeros } }, attributes: ['numero'], raw: true,
    });
    const existingSet = new Set(existingRows.map(r => r.numero));

    // Déjà envoyées avec succès → is_sent=1 (cas limite supprimée/re-téléchargée)
    const sentRows = await LogsAction.findAll({
      where: { numero_facture: { [Op.in]: numeros }, SendBy: { [Op.ne]: null }, erreur: false },
      attributes: ['numero_facture'], raw: true,
    });
    const sentSet = new Set(sentRows.map(r => r.numero_facture));
    ...
    invoices.forEach((inv, idx) => {
      const numero = inv && inv.numero;
      if (!numero || !inv.data) { results.push({ numero: numero || null, status: 'error', reason: 'champs manquants' }); return; }
      if (existingSet.has(numero) || seen.has(numero)) { results.push({ numero, status: 'already_downloaded' }); return; }
      seen.add(numero);

      // type effectif (comme saveDownloadedInvoice)
      let typeCode = inv.invoice_type_code || null;
      if (!typeCode && Array.isArray(inv.data) && inv.data.length > 0) {
        const f = inv.data[0] || {};
        const pos = f.point_of_sale || f.PointOfSale || f.pointOfSale;
        if (pos) typeCode = (pos === 'NPG') ? 'NPG_SIEGE_FACTURATION' : pos;
      }
      let verified = 0;
      if (Array.isArray(inv.data) && inv.data.length > 0 && inv.data[0].verified !== undefined) verified = inv.data[0].verified;

      const user = username || inv.username;
      // id unique garanti (évite les collisions de Date.now() côté client)
      const id = inv.id && !seen.has(`id:${inv.id}`) ? inv.id : `${baseTs}${idx}`;
      seen.add(`id:${id}`);
      ...
      toCreate.push({
        ...
        client: inv.client || 'Client inconnu',
        data: inv.data,
        verified,
        invoice_type_code: typeCode,
        is_sent: sentSet.has(numero),
      });
      logsToCreate.push({
        username: user, numero_facture: numero, download_date: new Date(),
        downloadBy: user, created_by: user,
      });
      results.push({ numero, status: 'created' });
    });

    if (toCreate.length > 0) {
      const t = await DownloadedInvoice.sequelize.transaction();
      try {
        await DownloadedInvoice.bulkCreate(toCreate, { transaction: t });
        await LogsAction.bulkCreate(logsToCreate, { transaction: t });
        await t.commit();
      } catch (e) {
        await t.rollback();
        throw e;
    ...
    res.json({
      ...
      skipped: results.filter(r => r.status === 'already_downloaded').length,
      errors: results.filter(r => r.status === 'error').length,
      results,
    });
  } catch (error) {
    console.error('Erreur téléchargement en masse:', error);
    res.status(500).json({ error: 'Erreur lors du téléchargement en masse', details: error.message });
  }
};
