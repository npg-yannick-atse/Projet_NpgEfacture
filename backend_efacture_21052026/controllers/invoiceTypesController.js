...
//   - downloaded : factures téléchargées NON ENCORE envoyées à la FNE
//                  (absence de ligne correspondante dans fne_invoices)
...
exports.stats = async (req, res) => {
    try {
        // 1) Téléchargements NON ENCORE envoyés à la FNE, par type.
        //    Une facture est "envoyée" dès qu'elle apparaît dans fne_invoices
        //    (succès API FNE OU saisie manuelle créant une fne_invoice MANUAL_*).
        const [downloadedRows] = await sequelize.query(`
            SELECT ${TYPE_CODE_EXPR} AS type_code, COUNT(di.id) AS cnt
