...
const getInvoiceDocument = async (req, res) => {
    ...
    try {
        ...
        let lignesFacture = result.XVBRP ? result.XVBRP.map(item => {
            ...
            // Calcul des montants
            const quantite = item.FKIMG ? parseFloat(item.FKIMG) : 0;
            ...
            if (kmein === 'KAR') {
                // Si KMEIN de la condition ZPR0 est KAR, prendre KBETR directement
                // MULTIPLIER PAR 1000 pour SAP (car frontend divise par 10)
                // Raw(50.88) * 1000 = 50880. Frontend / 10 = 5088 (Real)
                prixUnitaireHT = kbetr * 1000;
                console.log(`  Formule KAR (KMEIN=${kmein}): ${kbetr} * 1000 = ${prixUnitaireHT}`);
            } else if (kmein === 'ST') {
                // Si KMEIN de la condition ZPR0 est ST, appliquer KBETR × UMVKZ
                // MULTIPLIER PAR 1000 pour SAP
                prixUnitaireHT = kbetr * umvkz * 1000;
                console.log(`  Formule ST (KMEIN=${kmein}): ${kbetr} × ${umvkz} * 1000 = ${prixUnitaireHT}`);
            } else {
                // Même formule (KBETR * UMVKZ / 100) mais ensuite on multiplie par 1000
                // Donc équivalent à KBETR * UMVKZ * 10
                prixUnitaireHT = (kbetr * umvkz) * 10;
                console.log(`  Formule défaut (unité=${uniteArticle}): (${kbetr} × ${umvkz}) * 10 = ${prixUnitaireHT} (Multiplicateur x1000 intégré)`);
            }
            ...
            return {
                ...
                quantite: quantite,
                unite: item.VRKME || item.MEINS || '', // Utiliser VRKME en priorité, sinon MEINS
...
const resolveAvoirSap = async (req, res) => {
    ...
    try {
        ...
        if (avoirHeader.SFAKN && avoirHeader.SFAKN.trim()) {
            ...
            // Stratégie 4 : Fallback par KUNRG (même client)
            if (!initialInvoiceNumber && avoirHeader.KUNRG) {
                console.log(`Fallback: recherche FNE via sap_vbrk_header.KUNRG = ${avoirHeader.KUNRG}...`);
                const linkedHeaders = await SapVbrkHeader.findAll({
                    where: {
                        KUNRG: avoirHeader.KUNRG,
                        VBELN: { [Op.ne]: vbeln }
                    },
                    attributes: ['VBELN'],
                    order: [['FKDAT', 'DESC']],
                ...
                for (const hdr of linkedHeaders) {
                    const fneCheck = await FneInvoice.findOne({
                        where: { numero_facture: hdr.VBELN }
                    });
                    if (fneCheck) {
                        initialInvoiceNumber = hdr.VBELN;
                        console.log(`Facture initiale trouvée via KUNRG+FNE: ${initialInvoiceNumber}`);
                        break;
                    }
                }
            }
        ...
        // Récupérer les unités de la facture initiale depuis sap_vbrp_item (par MATNR)
        ...
        const initialVbrpItems = await SapVbrpItem.findAll({
            ...
            attributes: ['POSNR', 'MATNR', 'VRKME', 'MEINS'],
            raw: true
        });
        ...
        console.log('Items avoir:', avoirItems.map(it => ({ MATNR: it.MATNR, FKIMG: it.FKIMG, NETWR: it.NETWR, VRKME: it.VRKME, MEINS: it.MEINS })));
        ...
        for (const avoirItem of avoirItems) {
            ...
            if (fneItem) {
                // FKIMG est déjà disponible depuis l'étape 1
                const qty = Math.abs(parseFloat(avoirItem.FKIMG) || 0);
                ...
                console.log(`MATCH: avoir MATNR=${matnr} → FNE ref=${fneItem.reference} (qty=${qty}) unités avoir=${avoirUnitEff} initiale=${initialUnitEff} match=${unitMatch}`);
            } else {
