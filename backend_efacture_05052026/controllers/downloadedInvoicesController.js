...
exports.getAllDownloadedInvoices = async (req, res) => {
  ...
  try {
    ...
    // Why: l'ancien code faisait 2 findAll sur la même table (1 pour exclure les
    // succès, 1 pour calculer le statut des factures retournées). raw:true évite
    // l'hydratation Sequelize → moins de RAM et CPU.
    ...
    for (const log of allSentLogs) {
      const resp = parseApi(log.api_response);

      // Calcul "is success" — identique à l'ancien code
      let isSuccess;
      if (resp === null) isSuccess = true;
      else if (resp.success === true) isSuccess = true;
      else if (resp.is_manual) isSuccess = true;
      else if (resp.success === false) isSuccess = false;
      else isSuccess = !resp.error;

      if (isSuccess) successfulSentNumbers.push(log.numero_facture);
