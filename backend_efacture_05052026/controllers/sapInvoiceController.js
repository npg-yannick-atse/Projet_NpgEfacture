...
const resolveAvoirSap = async (req, res) => {
    ...
    try {
        ...
        for (const avoirItem of avoirItems) {
            ...
            // Snapshot complet de la ligne avoir SAP — quantités en CARTONS (FKIMG)
            // + CTS=FKIMG (cartons) et CLS=UMVKZ (pièces/carton) à titre informatif
            const avoirQtyCartons = Math.abs(parseFloat(avoirItem.FKIMG) || 0);
            ...
            allAvoirSapItems.push({
                ...
                quantity: avoirQtyCartons,                            // cartons (FKIMG)
                cts: avoirCts,
                cls: avoirCls,
            ...
            if (fneItem) {
                const qty = avoirQtyCartons; // quantité à rembourser exprimée en cartons (FKIMG)
