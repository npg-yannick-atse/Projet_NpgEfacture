...
exports.getAllDownloadedInvoices = async (req, res) => {
  ...
  try {
    ...
    // Un seul scan de logs_actions, exploité pour l'exclusion ET le statusMap.
    // Why: on s'appuie sur la colonne booléenne INDEXÉE `erreur` (posée à
    // l'insertion par actionLogger.js::isErrorResponse) au lieu de recharger et
    // JSON.parse la colonne TEXT volumineuse `api_response` pour 5k-50k logs à
    // CHAQUE ouverture de menu — c'était le goulot principal. Ici on ne lit que
    // 2 colonnes courtes (numero_facture, erreur). erreur=false ⟺ envoi réussi.
    const allSentLogs = await LogsAction.findAll({
      where: { SendBy: { [Op.ne]: null } },
      attributes: ['numero_facture', 'erreur'],
      order: [['created_on', 'DESC']], // pour que statusMap garde le PLUS RÉCENT
      raw: true,
    });

    const successfulSentNumbers = [];
    const statusMap = {};
    for (const log of allSentLogs) {
      if (!log.erreur) successfulSentNumbers.push(log.numero_facture);

      // statusMap : on garde le log le PLUS RÉCENT (DESC → premier rencontré).
      // Le plus récent en échec ⇒ 'failed' ; le plus récent en succès ⇒ null.
      if (!(log.numero_facture in statusMap)) {
        statusMap[log.numero_facture] = log.erreur ? 'failed' : null;
      }
    }

    perf(`scan logs_actions (${allSentLogs.length} logs envoyés, colonne erreur)`);

    if (req.query.includeSent !== 'true' && successfulSentNumbers.length > 0) {
    ...
    perf(`DownloadedInvoice.findAll (${downloadedInvoices.length} factures)`);
