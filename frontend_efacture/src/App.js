...
const FactureDisplay = ({
  ...
  lignesFacture = lignesFacture.map(ligne => ({
    ...
    PaymentMethod: ligne.PaymentMethod || 'check',
              ...
              {lignesFacture.map((ligne, index) => {
                ...
                return (
                  <TableRow key={index} hover>
                    ...
                    />
                    <EditableSelectCell
                      value={getModifiedValue(ligne.numeroFacture, 'PaymentMethod', ligne.PaymentMethod || 'check')}
                    ...
                    <TableCell align="right" sx={{ backgroundColor: '#ffffcc', borderRight: '1px solid #e0e0e0', borderBottom: '1px solid #e0e0e0' }}>{ligne.quantite ? Number(ligne.quantite).toLocaleString('fr-FR', { minimumFractionDigits: 0 }) : '0'}</TableCell> {/* Quantité */}
...
function MainApp() {
  ...
  const applyAllModifications = async () => {
    ...
    try {
      ...
      setPendingModifications({});
      alert(`${modifications.length} modification(s) appliquée(s) avec succès`);
    ...
    try {
      ...
      const modifiedInvoiceData = invoiceData.map(item => {
        ...
        return {
          ...
          paymentMethod: getModifiedValue(currentInvoiceNumber, 'PaymentMethod', item.PaymentMethod ?? item.payment_method ?? 'check'),
      ...
      const formattedInvoiceData = {
        ...
        paymentMethod: modifiedInvoiceData[0]?.paymentMethod || "check",
      ...
      if (isBlank(formattedInvoiceData.clientEmail))       missing.push('Email client (clientEmail)');
      ...
      if (response.ok) {
        ...
        try {
          ...
          // ENREGISTRER L'ÉCHEC dans les logs pour que l'indication s'affiche dans la liste
          try {
            await fetch(API_ENDPOINTS.LOGS.SEND, {
              method: 'POST',
              headers: {
              ...
              body: JSON.stringify({
                ...
                apiResponse: {
                  success: false,
                  status: response.status,
                  error: errorResponse.message || response.statusText || 'Erreur interne du serveur de signature'
                },
                fneResponseTimeMs: fneDurationMs
              })
            });
          ...
          return {
            success: false,
            error: errorResponse.message || 'Erreur inconnue',
            status: response.status,
  ...
  const handleOpenRefundModal = async (invoice) => {
    ...
    try {
      ...
      if (!invoiceNumber) {
        ...
        alert('Numéro de facture non trouvé');
        setIsLoadingFneInvoice(false);
        return;
      }

      // Récupérer les détails de la facture FNE
      const response = await fetch(`${API_ENDPOINTS.FNE_INVOICES.BY_SAP_NUMBER}/${invoiceNumber}`, {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
      ...
      if (result.success && result.data) {
        ...
        alert('Facture FNE non trouvée. Assurez-vous que la facture a été envoyée avec succès.');
      }
    } catch (error) {
      console.error('Erreur lors de la récupération de la facture FNE:', error);
      alert('Erreur lors de la récupération de la facture FNE');
    } finally {
      setIsLoadingFneInvoice(false);
    }
  };
  ...
  const handleSavePosSelections = async () => {
    try {
      ...
      if (response.ok) {
        alert('Points de vente mis à jour avec succès');
  ...
  */
  ...
  const rechercherFacture = async (overrideNumero = null) => {
    ...
    try {
          ...
          if (isAvoirDetected) {
            ...
            try {
              ...
              if (resolveResult.success) {
                ...
                if (resolveResult.avoir) {
                  setAvoirSapResult({ partial: true, ...resolveResult, avoirData: response.data });
                  // Si la facture initiale est connue mais absente de FNE, proposer son téléchargement
      ...
      if (err.response && err.response.status === 409) {
        setError("Cette facture a déjà été téléchargée et ENVOYÉE");
      } else {
  ...
  const handlePrintInvoiceWithLog = async (token, invoiceNumber) => {
    ...
    const cleanToken = token.replace('https://api.fne-ci.com/facture/', '');
    window.open(cleanToken, '_blank');
  };
  ...
  const handleManualRegister = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        alert(response.data.message || 'Opération effectuée avec succès');
      }
    } catch (err) {
  ...
  const processAndRegisterTemplateData = async (dataToProcess, initialSkippedLines = [], headers = []) => {
    ...
    try {
      ...
      for (const [, group] of groups.entries()) {
        ...
        if (isBlank(headerEmail)) headerMissing.push('Email');
        ...
        try {
          ...
          const lines = rows.map(row => ({
            ...
            payment_method: (() => {
              const pm = cleanValue(row.PaymentMethod).toLowerCase();
              if (pm === 'comptant') return 'cash';
              if (pm === 'transfert') return 'transfer';
              if (pm === 'cheque') return 'check';
              return pm;
            })(),
    ...
    reader.onload = (event) => {
      try {
        ...
        if (missingColumns.length > 0) {
          ...
          alert(`Colonnes manquantes dans le template: ${missingColumns.join(', ')}\n\nColonnes trouvées: ${headers.join(', ')}`);
          return;
        }
  ...
  const handleViewSentInvoice = async (sentInvoice) => {
    ...
    if (!targetInvoice) {
      try {
        ...
        alert("Impossible de charger les détails de cette facture.");
      } finally {
        setLoading(false);
      }
    }
    ...
    if (targetInvoice) {
      ...
      try {
        ...
        if (avoirInvoice) {
          ...
          alert("Impossible de charger les détails de cet avoir depuis SAP.");
        }
      } catch (e) {
        console.error("Erreur chargement avoir:", e);
        alert("Impossible d'afficher les détails de cet avoir.");
      } finally {
        setLoading(false);
      }
    } else {
      alert("Détails de la facture introuvables.");
    }
  };
  ...
  const handleDownloadInvoice = async () => {
    ...
    const looksLikeAvoir = ['ZRE', 'G2', 'S1'].includes(fkart);
    if (looksLikeAvoir) {
      if (!avoirSapResult || avoirSapResult.partial || avoirSapResult.success === false) {
        if (missingInitialInvoice?.numeroFacture) {
          alert(
            `Téléchargement bloqué : la facture initiale ${missingInitialInvoice.numeroFacture} de cet avoir n'est pas téléchargée et envoyée à la FNE.\n` +
            `Télécharge-la d'abord depuis la boîte de dialogue affichée.`
          );
        } else {
          alert(
            "Téléchargement bloqué : cet avoir ne peut pas être téléchargé tant que sa facture initiale n'a pas été téléchargée et envoyée à la FNE.\n" +
    ...
    try {
      ...
      if (sentCheckRes.ok) {
        ...
        if (sentCheckData.data && sentCheckData.data.length > 0) {
          ...
          if (isSent) {
            alert(`Impossible de télécharger la facture ${invoiceNumber} :\n\nCette facture a déjà été téléchargée et ENVOYÉE.`);
            return;
          }
        }
      }
  ...
  const handleToggleSelection = (id) => {
    ...
    setSelectedInvoiceIds(prev => {
      ...
      if (prev.length >= 15) {
        alert("Vous ne pouvez sélectionner que 15 factures maximum à la fois.");
        return prev;
      }
      return [...prev, id];
    });
  ...
  const handleSelectAll = () => {
    ...
    if (idsToSelect.length === 0) {
      alert("Aucune facture vérifiée et non envoyée n'est disponible pour la sélection.");
    } else if (availableInvoices.length > 15) {
      alert(`15 factures sur ${availableInvoices.length} ont été sélectionnées (limite maximale pour l'envoi).`);
    } else {
      alert(`${idsToSelect.length} factures ont été sélectionnées.`);
    }
  };
  ...
  const handleBulkSend = async () => {
    ...
    const confirm = window.confirm(`Voulez-vous envoyer les ${selectedInvoiceIds.length} factures sélectionnées ?`);
    if (!confirm) return;
  ...
  const handleSelectAllAndSend = async () => {
    ...
    if (ids.length === 0) {
      alert("Aucune facture vérifiée et non envoyée n'est disponible.");
      return;
    }
  ...
  const handleBulkDelete = async () => {
    ...
    const confirm = window.confirm(`Voulez-vous VRAIMENT supprimer les ${selectedInvoiceIds.length} factures sélectionnées ?\n\nCette action est irréversible.`);
    if (!confirm) return;
  ...
  const handleBulkDownload = async () => {
    ...
    for (let i = 0; i < foundInvoices.length; i++) {
      const inv = foundInvoices[i];

      // Si déjà téléchargée, en erreur, ou avoir non résolu, on saute
      if (inv.status === 'already_downloaded' || inv.status === 'success' || inv.status === 'avoir_unresolved') {
        setBulkDownloadProgress(prev => ({ ...prev, current: i + 1 }));
        continue;
      }

      try {
        // On passe directement l'objet facture complet (inv.data) à performDownload
        // ainsi que la résolution d'avoir précalculée (null pour les factures normales)
        await performDownload(inv.data, true, inv.avoirResolution); // true = silent mode
        // successCount++;
        ...
        console.error(`Erreur lors du téléchargement de ${inv.numero}:`, err);
        setFoundInvoices(prev => {
          const updated = [...prev];
    ...
    // Résumé des avoirs bloqués (leur facture initiale manque)
    const blockedAvoirs = foundInvoices
      .filter(inv => inv.status === 'avoir_unresolved' && inv.avoirResolution?.avoir?.factureInitiale)
      .map(inv => ({
        avoir: inv.numero,
        initial: inv.avoirResolution.avoir.factureInitiale,
      }));
  ...
  const handleDateRangeSelection = (numero) => {
    setSelectedDateRangeInvoices(prev => {
      ...
      if (prev.length >= 50) {
        alert("Vous ne pouvez sélectionner que 50 factures maximum à la fois.");
        return prev;
      }
      return [...prev, numero];
    });
  ...
  const handleDateRangeSelectAll = () => {
    ...
    if (available.length === 0) {
      alert("Aucune facture disponible pour la sélection (déjà téléchargées ou aucune trouvée).");
      return;
    }

    if (selectable.length > 50) {
      alert("50 factures ont été sélectionnées (limite maximale).");
    }
  ...
  // Gère le cas avoir : si la facture initiale manque, on la télécharge d'abord.
  const handleDownloadOneFromBulk = async (idx) => {
    ...
    try {
      ...
      // Cas avoir avec facture initiale non trouvée en FNE : proposer son téléchargement
      if (inv.avoirResolution && inv.avoirResolution.success === false && inv.avoirResolution.avoir?.factureInitiale) {
        setMissingInitialInvoice({
          numeroFacture: inv.avoirResolution.avoir.factureInitiale,
          numeroAvoir: inv.numero,
        });
        setFoundInvoices(prev => {
          const up = [...prev];
          up[idx] = { ...up[idx], status: 'avoir_unresolved' };
          return up;
        });
        return;
      }

      // Téléchargement normal (facture ou avoir avec résolution OK)
      await performDownload(inv.data, false, inv.avoirResolution); // false = non silent (affiche détail)
      setFoundInvoices(prev => {
        const up = [...prev];
  ...
  const handleDownloadSelectedDateRange = async () => {
    ...
    if (!window.confirm(`Voulez-vous télécharger les ${selectedDateRangeInvoices.length} factures sélectionnées ?`)) {
      return;
    }
  ...
  const AVOIR_FKART_CODES = ['ZRE', 'G2', 'S1'];
  const isAvoirType = (fkart) => {
    const v = (fkart || '').toString().trim().toUpperCase();
    return AVOIR_FKART_CODES.includes(v);
  };
  ...
  */
  ...
  const performDownload = async (invoiceToDownload = facture, silent = false, avoirResolutionOverride = null) => {
    ...
    // Ajouter les modifications inline aux données téléchargées
    ...
    if (invoiceNumber && Object.keys(allModifications).length > 0) {
      console.log('Application des modifications inline...');
      invoiceData.data = invoiceData.data.map(item => {
        const modifiedItem = { ...item };

        // Appliquer les modifications pour chaque champ
        if (allModifications[`${invoiceNumber}_ClientEmail`]) {
          console.log('Application ClientEmail:', allModifications[`${invoiceNumber}_ClientEmail`]);
          modifiedItem.clientEmail = allModifications[`${invoiceNumber}_ClientEmail`];
        }
        if (allModifications[`${invoiceNumber}_ClientPhone`]) {
          console.log('Application ClientPhone:', allModifications[`${invoiceNumber}_ClientPhone`]);
          modifiedItem.clientPhone = allModifications[`${invoiceNumber}_ClientPhone`];
        }
        if (allModifications[`${invoiceNumber}_Template`]) {
          console.log('Application Template:', allModifications[`${invoiceNumber}_Template`]);
          modifiedItem.template = allModifications[`${invoiceNumber}_Template`];
        } else {
          // Ajouter le champ template avec valeur par défaut s'il n'existe pas
          modifiedItem.template = 'B2B';
        }
        if (allModifications[`${invoiceNumber}_PaymentMethod`]) {
          console.log('Application PaymentMethod:', allModifications[`${invoiceNumber}_PaymentMethod`]);
          modifiedItem.paymentMethod = allModifications[`${invoiceNumber}_PaymentMethod`];
        } else {
          // Ajouter le champ paymentMethod avec valeur par défaut s'il n'existe pas
          modifiedItem.paymentMethod = 'mobile-money';
        }
        if (allModifications[`${invoiceNumber}_InvoiceType`]) {
          console.log('Application InvoiceType:', allModifications[`${invoiceNumber}_InvoiceType`]);
          modifiedItem.invoiceType = allModifications[`${invoiceNumber}_InvoiceType`];
        } else {
          // Ajouter le champ invoiceType avec valeur par défaut s'il n'existe pas
          modifiedItem.invoiceType = facture?.isRefund ? 'refund' : 'sale';
        }
        if (allModifications[`${invoiceNumber}_isRne`]) {
          console.log('Application isRne:', allModifications[`${invoiceNumber}_isRne`]);
          modifiedItem.isRne = allModifications[`${invoiceNumber}_isRne`];
        } else {
          // Ajouter le champ isRne avec valeur par défaut s'il n'existe pas
          modifiedItem.isRne = 'False';
        }
        if (allModifications[`${invoiceNumber}_PointOfSale`]) {
          const posVal = allModifications[`${invoiceNumber}_PointOfSale`];
          console.log('Application PointOfSale:', posVal);
          modifiedItem.pointOfSale = posVal;
        } else {
          // Ajouter le champ pointOfSale avec valeur par défaut s'il n'existe pas
          modifiedItem.pointOfSale = 'NPG_SIEGE_FACTURATION';
        }

        return modifiedItem;
      });

      console.log('Données de facture avec modifications inline:', invoiceData.data);
    }
  ...
  return (
    <Box sx={{ display: 'flex' }}>
        ...
        {isDetailViewMode && facture ? (
          ...
          <Container maxWidth="lg">
            <Box sx={{ display: 'flex', justifyContent: 'flex-end', mb: 2, gap: 2 }}>
              {(selectedPos === 'SURCCUSALE' || selectedPos === 'FACTURE_EXPORT') && (
                <>
                    ...
                    downloadedInvoices
                      ...
                      return (
                        <TableRow
                          ...
                          hover
                          sx={{
                            bgcolor: invoice.status === 'failed' ? '#FFA500' : isDownloadedAvoir ? '#fff8e1' : (isVerified ? 'inherit' : '#e3f2fd'),
                            '&.MuiTableRow-hover:hover': {
                              bgcolor: invoice.status === 'failed' ? '#FF8C00' : isDownloadedAvoir ? '#fff3e0' : (isVerified ? 'action.hover' : '#bbdefb')
                            }
                          }}
                        >
                          <TableCell padding="checkbox">
                          ...
                          <TableCell>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                              ...
                              {invoice.computedDetails?.numeroFacture || (invoiceDetails[invoice.id]?.numeroFacture) || 'Chargement...'}
                            </Box>
                          </TableCell>
                          <TableCell>
          ...
          <Container maxWidth={false}>
            ...
            <TableContainer component={Paper}>
              <Table>
                ...
                <TableBody>
                  {sentInvoices.length > 0 ? (() => {
                    ...
                    const pagedGroups = groups.slice(sentPage * sentRowsPerPage, (sentPage + 1) * sentRowsPerPage);

                    // Helper : rend une ligne (parent ou child)
                    const renderRow = (invoice, opts = {}) => {
                      const { isChild = false, hasChildren = false, expanded = false, onToggle, group } = opts;
                          ...
                          sx={{
                            backgroundColor: invoice.status === 'failed'
                              ...
                              : isChild
                                ...
                                : 'inherit',
                            '&:hover': {
                              backgroundColor: invoice.status === 'failed' ? '#FF8C00' : 'rgba(0, 0, 0, 0.04)'
                            },
                            borderLeft: isChild ? '3px solid #ed6c02' : 'none',
                          }}
                        >
                          ...
                          <TableCell sx={{ py: 0.5, px: 0.5, whiteSpace: 'nowrap' }}>
                              ...
                              <IconButton
                                ...
                                onClick={async () => {
                                  ...
                                  if (foundInvoice) {
                                    ...
                                    alert(`Facture non trouvée (${invoice.numero_facture}). Impossible de créer l'avoir.`);
                                  }
                                }}
                                color="warning"
                            ...
                            {/* 4. Bouton Saisie Manuelle (Pour les échecs OU les factures manuelles) */}
                            {(invoice.status === 'failed' || invoice.is_manual) && (
                              <IconButton
                                size="small"
                                onClick={() => {
                                  setManualFneData({
                                    numeroFacture: invoice.numero_facture || '',
                                    fneReference: invoice.reference || (invoice.api_response?.reference) || ''
                                  });
                                  setManualFneOpen(true);
                                }}
                                title="Saisie manuelle FNE"
                                color="info"
                              >
                    ...
                    return pagedGroups.flatMap((g) => {
                      ...
                      rows.push(renderRow(g.parent, { isChild: false, hasChildren, expanded, onToggle, group: g }));
                      if (expanded && hasChildren) {
                        g.children.forEach(c => rows.push(renderRow(c, { isChild: true })));
                      }
                      return rows;
                    });
                  })() : (
              ...
              <TablePagination
                component="div"
                count={(() => {
                  // Compte des groupes (= factures parent + avoirs orphelins sans parent visible)
                  ...
                  sentInvoices.forEach(i => {
                    if (i.invoice_type === 'refund') {
                      ...
                      if (!numeros.has(parentNum)) groups += 1;
                    }
                  });
                  return groups;
                })()}
      ...
      <Dialog
        ...
        PaperProps={{ sx: { borderRadius: 3, minWidth: 450 } }}
      >
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1, color: 'warning.dark' }}>
          <ReportProblemIcon color="warning" />
          Facture initiale non envoyée à la FNE
        </DialogTitle>
        <DialogContent>
          <Typography variant="body1" sx={{ mb: 2 }}>
            La facture initiale <strong>{missingInitialInvoice?.numeroFacture}</strong> liée à cet avoir n'a pas encore été envoyée à la FNE.
          </Typography>
          <Typography variant="body2" color="text.secondary">
            Pour pouvoir traiter l'avoir, vous devez d'abord télécharger et envoyer la facture initiale. Voulez-vous la télécharger maintenant ?
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button onClick={() => setMissingInitialInvoice(null)}>Annuler</Button>
          <Button
            variant="contained"
            color="warning"
            startIcon={<DownloadIcon />}
            onClick={() => {
              const num = missingInitialInvoice.numeroFacture;
              setMissingInitialInvoice(null);
              setAvoirSapResult(null);
              rechercherFacture(num);
            }}
          >
            Télécharger la facture initiale
          </Button>
        </DialogActions>
      </Dialog>
          ...
          <Button
            ...
            onClick={() => {
              if (!manualFneData.fneReference) {
                alert('Veuillez saisir une référence FNE');
                return;
              }
                      ...
                      <TableRow key={item.fne_item_id}>
                        ...
                        <TableCell align="right">
                          <TextField
                            ...
                            value={refundQuantities[item.fne_item_id] || 0}
                            onChange={(e) => {
